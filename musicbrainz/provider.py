...
class MusicbrainzProvider(MetadataProvider):
    ...
    async def get_artist_details(self, artist_id: str) -> MusicBrainzArtist:
        ...
        # === FALLBACK ===
        # Placeholder UUIDs (or genuinely unknown IDs) are not in the MB database;
        # return a stub so callers that depend on get_artist_details don't crash
        # on InvalidDataError and downstream providers can still run.
        self.logger.debug(
            "MusicBrainz: artist ID '%s' not found — returning placeholder", artist_id
        )
        return _make_placeholder_artist(artist_id)
    ...
    async def get_recording_details(self, recording_id: str) -> MusicBrainzRecording:
        ...
        return _make_placeholder_recording(recording_id)
    ...
    async def get_releasegroup_details(self, releasegroup_id: str) -> MusicBrainzReleaseGroup:
        ...
        # === FALLBACK ===
        self.logger.debug(
            "MusicBrainz: release-group ID '%s' not found — returning placeholder",
            releasegroup_id,
        )
        return _make_placeholder_release_group(releasegroup_id)
    ...
    async def get_release_group_by_track_name(
        ...
        if not (result := await self._search_release_groups_by_track_name(artist_name, track_name)):
            # === FALLBACK ===
            # No recordings matched — return a placeholder artist so downstream providers fire.
            self.logger.debug(
